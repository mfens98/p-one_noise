...
public:
    ...
    virtual G4VPhysicalVolume* Construct(){
		...
		G4Sphere* lSphere = new G4Sphere(lGlassSphere_name,0.0381*m,0.0451*m,0*degree,360*degree,0*degree,90*degree);
		...
		G4Sphere* rSphere = new G4Sphere(rGlassSphere_name,0.0381*m,0.0451*m,0*degree,360*degree,0*degree,90*degree);
...
public:
	K40coincPrimaryGeneratorAction() : G4VUserPrimaryGeneratorAction(){
		...
		G4ParticleDefinition* k40e = G4ParticleTable::GetParticleTable()->FindParticle( "e-" );


		genSource = new G4GeneralParticleSource();

		G4SingleParticleSource* k40glass = genSource->GetCurrentSource();
		k40glass->SetParticleDefinition(k40e);
		G4SPSPosDistribution* position = k40glass->GetPosDist();
		position->SetPosDisType("Point");

		G4cout << "HIHO" << G4RandFlat::shoot() << G4endl;
		G4SPSAngDistribution* angle = k40glass->GetAngDist();
		...
		G4SPSEneDistribution* energy = k40glass->GetEneDist();
	...
	virtual void GeneratePrimaries(G4Event* event){
		...
		G4double r = 0.007*std::pow(G4UniformRand(),1./3)+0.0381; //r_min=0.0381m r_max=0.0451m
		G4double normalization = std::pow((u*u + v*v + w*w),0.5);

		G4ThreeVector randpos = r*(G4ThreeVector(u,v,w))/normalization*m;
		G4double trans;
		if (randpos.z()>0){trans= +0.2441*m;}
		else if (randpos.z()<0){trans= -0.2441*m;}
		else {trans=-0.2441+(2*0.2441*G4RandFlat::shootBit())*m;}
		randpos = randpos+G4ThreeVector(0.,0.,trans);
		

		G4SingleParticleSource* k40glass = genSource->GetCurrentSource();
    	k40glass->SetParticlePolarization(G4RandomDirection());
    	k40glass->SetParticleTime(G4UniformRand()*ms);
    	k40glass->GetPosDist()->SetCentreCoords(randpos);
    	k40glass->GeneratePrimaryVertex(event);
...
public:
	...
	virtual void UserSteppingAction(const G4Step* step){
		...
		static int        up         = 0; 
		static int        down       = 0;
		static G4double   quantum_eff = 1., uptime, downtime, uplen, downlen, uene1,dene1;
...
int main(int argc,char** argv){
	...
	if (var["ui"] != 0.0){
		...
		G4long randnum = CLHEP::RandPoisson::shoot(3.);
		//G4int numparticles = randnum;
		G4cout << "Run will consist of " << randnum << " particles" << G4endl;
		u->ApplyCommand(G4String("/run/beamOn ") +G4String(to_string((int)randnum)));
	}
	delete visManager;
	delete runManager;
}
