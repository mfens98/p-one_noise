    ...
    for i in range(len(short)):
        ...
        if len(potential_list)>0: 
            for j in range(len(potential_list)):
                ...
                if (sdown):
                    data_list.append([potential_list[j],t0,-diff]) #uptime, downtime, up-down
                else: data_list.append([t0,potential_list[j],diff])
...
def write_data(data_arr,ms_vals,filepath):
    ...
    np.savetxt("/data/p-one/mens/Coincidence_analysis/parsing/pulse_check/msCounts/"+filename+"_ms.txt",ms_vals,delimiter="\t")
    np.savetxt("/data/p-one/mens/Coincidence_analysis/parsing/pulse_check/coincidenceFiles/"+filename+"_coincs.txt",data_arr,delimiter="\t")
